...
class RealSenseReader:
    ...
    def get_image(self,wait_frames=3):
        if not self.running:
            self.start()
        time.sleep(0.1)
...
class WasdInterface(object):
    ...
    def __init__(self, robot):
        ...
        algo='retrieval' #bc retrieval
        self.agent,self.encoder = load_agent(task=self.task_name,algo=algo)
    ...
    def get_ob_buff(self,):
        ...
        image_state = self.realsense_state
        obs_dict={
            'robot_pos':robot_pos,
            'robot_rot':robot_rot,
            'elem_pos':elem_pos,
            ...
            'rgb_img':image_state['rgb'],
            'dep_img':image_state['depth']
        }
            ...
            try:
                while not self._exit_check.kill_now:
                    ...
                    try:
                        ...
                        if (self.agent_flag and cmd ==-1): #in [ord('w'),ord('a'),ord('s'),ord('d'),
                                                        #ord('u'),ord('j'),ord('h'),ord('k'),ord('m'),ord('n')] ):
                            
                            obs,obs_dict = self.get_visual_feature()
                            #obs = obs.reshape(1,-1)
                            #cmd_chr = chr(cmd)
                            action = self.agent.get_action(obs)#Action_map[cmd_chr][:6] #
                            #action = np.array(action)
                            self.excuate_act_hand(action)
                            time.sleep(2) #COMMAND_INPUT_RATE
    ...
    def _rs_state_str(self):
        ...
        return f'Agent step number :  {self.agent_step_num}\n \
            ...
            #State includes {state.keys()}'  # get rid of STATE_ prefix
